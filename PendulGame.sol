...
contract PendulumGame {
        ...
        onlyPlayer1(_sessionId) 
    {
        ...
        require(keccak256(abi.encodePacked(_word)) == session.wordHash, "Revealed word does not match the hash");
        ...
        onlyPlayer2(_sessionId) 
    {
        ...
        bool correct = false;
        if (keccak256(abi.encodePacked(_guess)) == keccak256(abi.encodePacked(session.word))) {
            correct = true;
            ...
            if (!session.guessedLetters[guessedLetter]) {
                ...
                if (bytes(session.word).length > 0 && contains(session.word, guessedLetter)) {
                    correct = true;
                } else {
                    session.guessesLeft--;
                }
        ...
        emit GuessMade(_sessionId, msg.sender, _guess, correct, session.guessesLeft);
